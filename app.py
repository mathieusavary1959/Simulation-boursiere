...
# --- CONNEXION BASE DE DONNÉES ET GESTION DES VERROUS ---
conn = st.connection("sqlite", type="sql", url="sqlite:///portfolio.db")


def init_db():
    with conn.session as session:
        # Activer le mode WAL pour autoriser les accès simultanés sans bloquer SQLite
...
# --- OPTIMISATION DU CACHE PRIX (SHARED CACHE 30s) ---
@st.cache_data(ttl=30, show_spinner=False)
def obtenir_prix_actuel(ticker: str) -> float:
    """Conserve le prix en mémoire globale 30s pour tous les élèves."""
    ticker = ticker.strip().upper()
    try:
        data = yf.Ticker(ticker)
        # Essai rapide
        prix = data.fast_info.get("lastPrice") or data.fast_info.get(
            "regularMarketPrice"
        )

        if prix is None or pd.isna(prix) or prix == 0:
            hist = data.history(period="1d")
            if not hist.empty:
                prix = hist["Close"].iloc[-1]

        return round(float(prix), 2) if prix else 0.0
    except Exception:
        return 0.0
...
with tab_buy:
    ...
    if ticker_input:
        ...
        if prix_actuel_buy > 0:
            ...
            if st.button("🛒 Confirmer l'Achat", use_container_width=True):
                if cash_actuel >= cout_total:
                    ...
                    try:
                        ...
                        st.error(
                            "Le serveur est très sollicité, veuillez réessayez la transaction."
                        )
                else:
                    st.error(
            ...
            st.warning(
                f"Saisie en cours ou cours temporairement indisponible pour '{ticker_input}'."
            )

# ==============================================================================
# ONGLET 2 : POSITIONS ET IMPRESSION (RAFRAÎCHISSEMENT AUTO TOUTES LES 15S)
# ==============================================================================
with tab_port:
    st.markdown(
        """
    ...
    # Rafraîchissement stabilisé à 15s pour préserver le serveur
    ...
    def afficher_positions_live():
        ...
        if not p_all.empty:
            ...
            for _, r in p_all.iterrows():
                ...
                pa = obtenir_prix_actuel(tk) or pm
                val = sh * pa
                pnl = (pa - pm) * sh
                pnl_pct = ((pa - pm) / pm * 100) if pm > 0 else 0

                pnl_color = "#10B981" if pnl >= 0 else "#EF4444"
                options_vente[f"{tk} ({sh} disponible(s))"] = (tk, sh, pa)
            ...
            with col_v3:
                ...
                if st.button(
                    ...
                    try:
                        ...
                        st.error("Serveur occupé, veuillez réessayer la vente.")
        else:
